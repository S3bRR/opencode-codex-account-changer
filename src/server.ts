...
import { credential, readAccounts, readSelection, writeSelection, type OAuthCredential } from "./accounts.js"
...
function request(source: Request, url: string, headers: Headers) {
...
const server: Plugin = async (input) => {
  ...
  let browserFlow: Awaited<ReturnType<typeof browser>> | undefined

  async function setAuth(id: string, auth: OAuthCredential) {
    await input.client.auth.set({ path: { id }, body: auth, throwOnError: true })
  }

  async function save(auth: OAuthCredential) {
    if (!auth.accountId) throw new Error("OpenAI did not return a ChatGPT account ID")
    await setAuth(`openai/${auth.accountId}`, auth)
    return auth
  }

  async function success(tokens: Awaited<ReturnType<typeof refresh>>) {
  ...
  return {
    ...
    auth: {
      ...
      async loader(getAuth) {
        if (!credential(await getAuth())) return {}
        ...
        return {
          ...
          async fetch(input: RequestInfo | URL, init?: RequestInit) {
            ...
            let auth: OAuthCredential | undefined
            ...
            auth ??= canonical
            if (!auth) return fetch(request(source, source.url, headers))
            ...
            const selectionChanged =
              selected !== undefined && auth.accountId === selected && (await readSelection().catch(() => undefined)) === selected
            if (canonicalChanged || selectionChanged) await setAuth("openai", auth)
...
export default { id: "codex-account-changer", server } satisfies PluginModule
