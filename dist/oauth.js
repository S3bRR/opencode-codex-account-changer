...
async function post(path, body, parent) {
    ...
    if (!response.ok)
        throw new Error(`OpenAI OAuth request failed (${response.status})`);
    return response.json();
}
export function toCredential(tokens, previous) {
...
export function refresh(refreshToken, parent) {
    return post("/oauth/token", new URLSearchParams({ grant_type: "refresh_token", refresh_token: refreshToken, client_id: CLIENT_ID }), parent);
}
async function pkce() {
...
export async function browser(parent) {
    const codes = await pkce();
    const state = Buffer.from(crypto.getRandomValues(new Uint8Array(32))).toString("base64url");
    let server;
    let cancel;
    const tokens = new Promise((resolve, fail) => {
        let settled = false;
        const finish = (action) => {
            if (settled)
                return;
        ...
        server = createServer(async (request, response) => {
            ...
            if (url.searchParams.get("state") !== state)
                return void response.writeHead(400).end(page("Invalid OAuth state"));
            ...
            try {
                const result = await post("/oauth/token", new URLSearchParams({
    ...
    void tokens.catch(() => { });
    await new Promise((resolve, fail) => server.once("error", (cause) => {
        cancel(cause);
        fail(cause);
...
export async function device(parent) {
    const challenge = await post("/api/accounts/deviceauth/usercode", { client_id: CLIENT_ID }, parent);
    return {
        ...
        async tokens() {
            ...
            while (Date.now() < expires) {
                ...
                if (response.ok) {
                    ...
                    return post("/oauth/token", new URLSearchParams({
                        grant_type: "authorization_code",
